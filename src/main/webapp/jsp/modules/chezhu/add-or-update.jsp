            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑车主</li>
                    </ol>
                </div>
            </div>
        </div>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input id="chezhuIdNumber" name="chezhuIdNumber" class="form-control"
                                           onchange="chezhuIdNumberChickValue(this)"  placeholder="身份证号">
                                </div>
                                <div class="form-group col-md-6">
                                    <label>手机号</label>
                                    <input id="chezhuPhone" name="chezhuPhone" class="form-control"
                                           onchange="chezhuPhoneChickValue(this)"  placeholder="手机号">
                                </div>
                                    <div class="form-group col-md-6">
                                        <label>车牌号</label>
                                   ...
                                   <div class="form-group col-md-6">
                                       ...
                                       <input name="file" type="file" id="chezhuPhotoupload"
                                              class="form-control-file">
                                       <input name="chezhuPhoto" id="chezhuPhoto-input" type="hidden">
                                   </div>
...
<script>
    ...
    function upload() {
        ...
        $('#chezhuPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('chezhuPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('chezhuPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("chezhu/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addchezhu');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function chezhuIdNumberChickValue(e){
        ...
        function chezhuPhoneChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                chezhuName: "required",
                sexTypes: "required",
                chezhuIdNumber: "required",
                chezhuPhone: "required",
                chezhuChepaiNumber: "required",
                chezhuPhoto: "required",
            },
            messages: {
                chezhuName: "姓名不能为空",
                sexTypes: "性别不能为空",
                chezhuIdNumber: "身份证号不能为空",
                chezhuPhone: "手机号不能为空",
                chezhuChepaiNumber: "车牌号不能为空",
                chezhuPhoto: "照片不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addchezhu = window.sessionStorage.getItem("addchezhu");
        if (addchezhu != null && addchezhu != "" && addchezhu != "null") {
            window.sessionStorage.removeItem('addchezhu');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("chezhu/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
